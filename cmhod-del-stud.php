...
if(isset($_POST['submit'])){
  ...
  $user = $_POST['user'];
$res = mysqli_query($db_handle,"SELECT * FROM users WHERE user='$user' AND branch='CM'");
if(mysqli_num_rows($res)<1)
{
	$msg = "Student doesnt exist or not from CM.";
}
else{
...
<br><br>
  ...
  <table class="reference" style="width:35%">
    <form action="cmhod-del-stud.php" method="POST">
  	<tr>
      <td>Enrollment No. </td>
  ...
  <br><br>
<?php if($msg){ echo "$msg"; } ?>
<table class="reference" style="width:40%">
<tr>
	<th>No</th>
...
<?php
		...
		$SQL = "SELECT * FROM users WHERE branch='CM' AND user!='admin' AND user!='cmhod' AND user!='ifhod' ORDER BY user ASC ";
		...
		while ($db_field = mysqli_fetch_assoc($result))
		{
			...
			$j = $i + 1;
			print("<td align='center' width = '70px'><form action='cmhod-del-stud.php' method='POST'><input type='hidden' name='user' value=".$user."><input type='submit' name='submit' value='Delete'></form></tr>");
			$i = $i + 1;
		}
?>
</table>
