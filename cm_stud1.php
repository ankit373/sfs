...
if($chk=="1")
{
		...
		if(isset($_POST['submit']))
		{
			...
			$re = mysqli_query($db_handle,"SELECT * FROM ques");
			$questions = array();
			$qid = array();
			...
			$que_count = count($questions);
					...
					$a1 = array();
					$i = 0 ;
					$ressub = mysqli_query($db_handle,"SELECT * FROM `$sub_table` WHERE year='$db_year'") or die();
					...
					$j = 0;
						...
						for($i=0;$i<count($a1);$i++)
						{
							...
							for($j=0;$j<count($questions);$j++)
							{
								...
								if($j == 0)
								{
									if($i==0)
									{
												if(mysqli_query($db_handle,"INSERT INTO `$feed` (`u_id`,`subject`,`message`,`year`,`$questions[$j]`) VALUES('$uid','$a1[$i]','$message','$db_year','$element')"))
												{
														$flag = 1;
												}
												else{
									...
									{
										if(mysqli_query($db_handle,"INSERT INTO `$feed` (`u_id`,`subject`,`message`,`year`,`$questions[$j]`) VALUES('$uid','$a1[$i]','','$db_year','$element')"))
										{
											$flag = 1;
										}
										else{
								...
								{
									if(mysqli_query($db_handle,"UPDATE `$feed` SET `$questions[$j]`='$element' WHERE `u_id`='$uid' AND `subject`='$a1[$i]'"))
									{
										$flag = 1;
									}
									else{
						...
						if($flag == 1)
						{
							mysqli_query($db_handle,"UPDATE users SET chk='1' WHERE user='$user'");
...
<tr>
	...
	<?php
		$que_data = mysqli_query($db_handle,"SELECT * FROM ques");
		$no = 1;
		$i = 0;
		$subjects = mysqli_query($db_handle,"SELECT * FROM `$sub_table` WHERE year='$db_year'");
		while($db_field=mysqli_fetch_assoc($que_data))
		{
			$a = $db_field['question'];
			print "<tr><td>$no</td>";
			print "<td>$a</td>";
			for($j=0;$j<mysqli_num_rows($subjects);$j++)
			{
