...
{
	include 'config/config.php';
	$res = mysqli_query($db_handle,"SELECT * FROM `$feed` WHERE subject='$subject'");
	$values = Array();
	$i = 0;
	$sum = $total = 0;
...
if(isset($_POST['year'])){
	$year = $_POST['year'];
	$res = mysqli_query($db_handle,"SELECT * FROM `$sub_table` WHERE year='$year'");
	$subjects = array();
	$i=0;
	while($db_field = mysqli_fetch_assoc($res))
	{
	...
	$res = mysqli_query($db_handle,"SELECT * FROM ques");
	$questions = array();
	$i=0;
	while($db_field = mysqli_fetch_assoc($res))
	{
	...
	$res1 = mysqli_query($db_handle,"SELECT * FROM users WHERE year='$year' AND chk='1' AND branch='$branch'");
