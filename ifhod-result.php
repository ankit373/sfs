...
function per($question,$subject)
{
	...
	$res = mysqli_query($db_handle,"SELECT * FROM `if_feed` WHERE subject='$subject'");
	$values = Array();
	$i = 0;
	$sum = $total = 0;
...
<?php
	if(isset($_POST['year'])){
		$year = $_POST['year'];
		$res = mysqli_query($db_handle,"SELECT * FROM `if_subject` WHERE year='$year'");
		$subjects = array();
		$i=0;
		while($db_field = mysqli_fetch_assoc($res))
		{
		...
		$res1 = mysqli_query($db_handle,"SELECT * FROM users WHERE year='$year' AND chk='1' AND branch='IF'");
